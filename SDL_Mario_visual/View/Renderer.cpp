...
{
	ListIterator<Object*> i = objects_.begin();
	float w,h;
...
{
	ListIterator<SdlString*> i = text_.begin();
	while(i != text_.end())
	{
		if(((*i)->isRendering()))
		{
...
void Renderer::add(Object* object)
{
	...
	for(ListIterator<Object*> i = objects_.begin(); i != objects_.end(); ++i)
	{
...
ListIterator<Object*>
Renderer::removeObject(ListIterator<Object*> i)
{
	return objects_.erase(i);
}

ListIterator<SdlString*>
Renderer::removeText(ListIterator<SdlString*> i)
{
...
void Renderer::initCamera()
{
	...
	if(camera_ == nullptr)
	{
		printf("Cannot create cameraTexture: %s\n", SDL_GetError());
		throw *(new ApplicationError);
	}
}
